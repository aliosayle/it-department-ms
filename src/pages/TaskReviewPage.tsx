...
import { portalCreateTask, portalReviewTask, portalUploadTaskAttachment } from '@/api/mutations'
...
export function TaskReviewPage() {
  ...
  const createdByUserId = userId || users[0]?.id || ''

  const create = async () => {
    const dueStr = dueDate ? dueDate.toISOString().slice(0, 10) : null
    const r = await portalCreateTask({
      title,
      description,
      assignedToUserId,
  ...
  const review = async (decision: 'approved' | 'changes_requested') => {
    const reviewer = userId || users[0]?.id || ''
    const r = await portalReviewTask({ taskId: selectedTaskId, decision, comment, reviewerUserId: reviewer })
    setMsg(r.ok ? 'Review saved.' : r.error)
  }

  const onFile = async (file: File | null) => {
    if (!file || !selectedTaskId) return
    const uploader = userId || users[0]?.id || ''
    ...
    const r = await portalUploadTaskAttachment({
  ...
  return (
    <div className="form-page form-page--wide">
      ...
      <SelectBox
        ...
        onValueChanged={(e) => setAssignedToUserId(String(e.value ?? ''))}
      ...
      <SelectBox
        ...
        onValueChanged={(e) => setReviewerUserId(String(e.value ?? ''))}
        placeholder="Reviewer"
      />
      <DateBox label="Due date" value={dueDate} onValueChanged={(e) => setDueDate((e.value as Date | null) ?? null)} />
      ...
      <SelectBox
        ...
        onValueChanged={(e) => setSelectedTaskId(String(e.value ?? ''))}
